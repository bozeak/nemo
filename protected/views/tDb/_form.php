<div class="form">
    ...
    <div class="row">
        ...
        echo $form->dropDownList($model,'subdiv',CHtml::listData(Subdiv::model()->findAll(),'id','name'),
            array(
                ...
                'ajax'=> array(
                    'type' => 'POST',
                    'url'=>CController::createUrl('tDb/dynamicresponsabil'),
                    'update'=>'#'.CHtml::activeId($model,'responsabil'),
                    'data'=>array('subdiv'=>'js:this.value'),
                )
            ));
    ...
    <div class="row">
        <?php echo $form->labelEx($model,'nr_etaje'); ?>
        <?php echo $form->textField($model,'nr_etaje',array('size'=>60,'maxlength'=>255)); ?>
        <?php echo $form->error($model,'nr_etaje'); ?>
    </div>

    <div class="row">
        <?php echo $form->labelEx($model,'suprafata'); ?>
        <?php echo $form->textField($model,'suprafata',array('size'=>60,'maxlength'=>255)); ?>
        <?php echo $form->error($model,'suprafata'); ?>
    </div>
    ...
    <div class="row">
        ...
        <?php echo $form->DropDownList($model,'responsabil',CHtml::listData(Responsabil::model()->findAllByAttributes(array('subdiv'=>$model->subdiv)),'id','fullname'),
            array('prompt'=>'Alegeți responsabilul'));
        ?>
